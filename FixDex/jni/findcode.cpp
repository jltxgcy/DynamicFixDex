...
#include <dlfcn.h>
#include "dexFile.h"
#include "com_example_fixdex_FindCode.h"
...
static int nativeParserDex(const char *className, const char *methodName)
{
	void *base = NULL;
	int module_size = 0;
	char filename[512];
	...
	for(int i=0; i<2; i++){
		sprintf(filename, "/mnt/sdcard/payload/ForceApkObj.dex");
...
JNIEXPORT jint JNICALL Java_com_example_fixdex_FindCode_findCode
  (JNIEnv *env , jclass jclazz, jstring className, jstring methodName) {
	...
	return nativeParserDex(classNameLocal, methodNameLocal);
}
